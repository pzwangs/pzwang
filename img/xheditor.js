...
function openreg()
{
	...
	window.open("w.asp?act=reg&reggoto=","regwindow","toolbar=no,resizable=no,directories=no,status=yes,scrollbars=auto,menubar=no,width=380,height=300,left="+x+",top="+y);
}
...
$(function(){$(".entry-thumbnail").hover(function(){$(".list-sum",this).stop().animate({top:2},300);},function(){$(".list-sum",this).stop().animate({top:190},300);})});
$(function(){$("#content > div").addClass("splitted");$(".wp-pagenavi").removeClass("splitted")});
$(function(){$(".zoom").fancybox({'overlayShow': false,'transitionIn': 'elastic','transitionOut': 'elastic','hideOnContentClick': true,'padding':8,'autoScale':false});});
...
var htmlLink='<div>链接地址: <input type="text" id="xheLinkUrl" value="http://" class="xheText" /></div><div style="display:none">链接文字: <input type="text" id="xheLinkText" value="" class="xheText" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="确定" /></div>';
var htmlImg='<div>图片文件: <input type="text" id="xheImgUrl" value="http://" class="xheText" /></div><div>替换文本: <input type="text" id="xheImgAlt" /><div><div style="text-align:right;"><input type="button" id="xheSave" value="确定" /></div>';
var htmlFlash='<div>视频Flash: <input type="text" id="xheFlashUrl" value="http://" class="xheText" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="确定" /></div>';
var htmlMedia='<div>音乐文件: <input type="text" id="xheMediaUrl" value="http://" class="xheText" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="确定" /></div>';
...
var htmlAbout='<div style="font:12px Arial;width:245px;word-wrap:break-word;word-break:break-all;"><p><a href="http://www.mihao.net/" target="_blank">米号微博</a> v1.1.0&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="http://xheditor.com/" target="_blank">xhEditor</a>编辑器 v1.1.6</p></div>';
...
$.xheditor=function(textarea,options)
{
	...
	var headHTML='',bodyClass='',skinPath=editorRoot+'xheditor_skin/'+settings.skin+'/',arrEmots=itemEmots,urlType=settings.urlType,urlBase=settings.urlBase,emotPath=settings.emotPath,emotPath=emotPath?emotPath:editorRoot+'emot/',selEmotGroup='';
	...
	this.focus=function()
	{
		...
		if(isIE){
			var rng=_this.getRng();
			if(rng.parentElement&&rng.parentElement().ownerDocument!==_doc)_this.setCursorFirst();
		}
		return false;
	}
		...
		else
		{
			...
			sHtml=sHtml.replace(/(<(\w+))((?:\s+[\w-:]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*(\/?>)/g,function(all,left,tag,attr,right){
				...
				attr=attr.replace(/\s+([\w-:]+)\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/g,function(all,n,v){
					...
					else if(n.match(/^((_xhe_|_moz_|_webkit_)|jquery\d+)/i))return '';					
					else if(saveValue&&n.match(/^(src|href)$/i))return ' '+n+'="'+saveValue+'"';
					else if(isSafari&&n==='style'){
						v=v.replace(/(^|;)\s*(font-size)\s*:\s*([a-z-]+)\s*(;|$)/i,function(all,left,n,v,right){
							var t,s;
							for(var i=0;i<arrFontsize.length;i++)
							{
	...
	this.formatXHTML=function(sHtml,bFormat){
		...
		var emptyTags = makeMap("area,base,basefont,br,col,frame,hr,img,input,isindex,link,meta,param");
		var blockTags = makeMap("address,applet,blockquote,button,center,dd,del,dir,div,dl,dt,fieldset,form,frameset,h1,h2,h3,h4,h5,h6,hr,iframe,ins,isindex,li,map,menu,noframes,noscript,object,ol,p,pre,script,table,tbody,td,tfoot,th,thead,tr,ul");
		...
		if(sHtml.length>nextIndex)onText(sHtml.substring(nextIndex,sHtml.length ));
		onEndTag();
	...
	this.toggleSource=function(state)
	{
		...
		if(!bSource)
		{
			...
			if(!isOpera)cursorPos=sHtml.substring(0,cursorPos).replace(/\r/g,'').length;
		...
		jSave.click(function(){
			...
			if(url!==''&&url!=='http://')
			{
				...
				else
				{
					...
					else if(sText&&!isSafari)jParent.text(sText);
	...
	this.uploadInit=function(jText,toUrl,upext)
	{
		...
		{
			if(is(arrMsg,'string'))arrMsg=[arrMsg];
	...
	this.startUpload=function(fromFiles,toUrl,limitExt,onUploadComplete)
	{
		...
		{
			if(!fileList)fileList=fromFiles;
		...
		if(panelState)bShowPanel=false;
		...
		function onUploadCallback(sText,bFinish)
		{
			...
			if(bFinish&&bOK)onUploadComplete(arrMsg);
			return bOK;
		}
	}
	...
	this.showModal=function(title,content,w,h,onRemove)
	{
		...
		if(isIE&&browerVer===6.0)jHideSelect=$('select:visible').css('visibility','hidden');
	...
	this.showDialog=function(content)
	{
		...
		if(!isIE)setTimeout(function(){jDialog.find('input[type=text],textarea').filter(':visible').filter(function(){return $(this).css('visibility')!=='hidden';}).eq(0).focus();},10);
	}
	this.showPanel=function(content)
	{
		...
		if((x+_jPanel.outerWidth())>document.body.clientWidth)x-=(_jPanel.outerWidth()-_jPanelButton.outerWidth());
...
$(function(){
	...
	$.fn.val=function(value)
	{
		...
		return _this.each(function(){if(editor=this.xheditor)editor.setSource(value);else _this.oldVal(value);});
	}
...
function ubb2html(f){function o(g){return g!=null&&g!=""?!isNaN(g):false}var b=String(f),p=[],n=0;b=b.replace(/[<>&"]/g,function(g){return{"<":"&lt;",">":"&gt;","&":"&amp;",'"':"&quot;"}[g]});b=b.replace(/\r?\n/g,"<br />");b=b.replace(/\[code\s*(?:=\s*([^\]]+?))?\]([\s\S]*?)\[\/code\]/ig,function(g){n++;p[n]=g;return"[\tubbcodeplace_"+n+"\t]"});b=b.replace(/\[(\/?)(b|u|i|s|sup|sub)\]/ig,"<$1$2>");b=b.replace(/\[color\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/ig,'<font color="$1">');b=b.replace(/\[size\s*=\s*(\d+?)\s*\]/ig,
...
'<img src="$1" alt="" />');b=b.replace(/\[img\s*=([^,\]]*)(?:\s*,\s*(\d*%?)\s*,\s*(\d*%?)\s*)?(?:,?\s*(\w+))?\s*\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*)?\s*\[\/img\]/ig,function(g,i,a,k,m,t){g='<img src="'+t+'" alt="'+i+'"';m=m?m:!o(a)?a:"";if(o(a))g+=' width="'+a+'"';if(o(k))g+=' height="'+k+'"';if(m)g+=' align="'+m+'"';g+=" />";return g});b=b.replace(/\[emot\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\/\]/ig,'<img emot="$1" />');b=b.replace(/\[url\]\s*(((?!")[\s\S])*?)(?:"[\s\S]*?)?\s*\[\/url\]/ig,'<a href="$1">$1</a>');
...
function(g,i,a,k){i||(i=480);a||(a=400);return'<embed type="application/x-shockwave-flash" src="'+k+'" wmode="opaque" quality="high" bgcolor="#ffffff" menu="false" play="true" loop="true" width="'+i+'" height="'+a+'"/>'});b=b.replace(/\[media\s*(?:=\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*(\d+)\s*)?)?\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/media\]/ig,function(g,i,a,k,m){i||(i=480);a||(a=400);return'<embed type="application/x-mplayer2" src="'+m+'" enablecontextmenu="false" autostart="'+(k=="1"?"true":"false")+
'" width="'+i+'" height="'+a+'"/>'});b=b.replace(/\[table\s*(?:=\s*(\d{1,4}%?)\s*(?:,\s*([^\]"]+)(?:"[^\]]*?)?)?)?\s*\]/ig,function(g,i,a){g="<table";if(i)g+=' width="'+i+'"';if(a)g+=' bgcolor="'+a+'"';return g+">"});b=b.replace(/\[tr\s*(?:=\s*([^\]"]+?)(?:"[^\]]*?)?)?\s*\]/ig,function(g,i){return"<tr"+(i?' bgcolor="'+i+'"':"")+">"});b=b.replace(/\[td\s*(?:=\s*(\d{1,2})\s*,\s*(\d{1,2})\s*(?:,\s*(\d{1,4}%?))?)?\s*\]/ig,function(g,i,a,k){return"<td"+(i>1?' colspan="'+i+'"':"")+(a>1?' rowspan="'+a+'"':
"")+(k?' width="'+k+'"':"")+">"});b=b.replace(/\[\/(table|tr|td)\]/ig,"</$1>");b=b.replace(/\[\*\]((?:(?!\[\*\]|\[\/list\]|\[list\s*(?:=[^\]]+)?\])[\s\S])+)/ig,"<li>$1</li>");b=b.replace(/\[list\s*(?:=\s*([^\]"]+?)(?:"[^\]]*?)?)?\s*\]/ig,function(g,i){var a="<ul";if(i)a+=' type="'+i+'"';return a+">"});b=b.replace(/\[\/list\]/ig,"</ul>");for(f=1;f<=n;f++)b=b.replace("[\tubbcodeplace_"+f+"\t]",p[f]);return b=b.replace(/(^|<\/?\w+(?:\s+[^>]*?)?>)([^<$]+)/ig,function(g,i,a){return i+a.replace(/[\t ]/g,
function(k){return{"\t":"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"," ":"&nbsp;"}[k]})})}
function html2ubb(f){function o(h){var c;if(c=h.match(/\s*rgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i)){for(h=(c[1]*65536+c[2]*256+c[3]*1).toString(16);h.length<6;)h="0"+h;h="#"+h}return h=h.replace(/^#([0-9a-f])([0-9a-f])([0-9a-f])$/i,"#$1$1$2$2$3$3")}var b={"xx-small":1,"8pt":1,"x-small":2,"10pt":2,small:3,"12pt":3,medium:4,"14pt":4,large:5,"18pt":5,"x-large":6,"24pt":6,"xx-large":7,"36pt":7},p=/\s+src\s*=\s*(["']?)\s*(.+?)\s*\1(\s|$)/i,n=/\s+width\s*=\s*(["']?)\s*(\d+(?:\.\d+)?%?)\s*\1(\s|$)/i,
g=/\s+height\s*=\s*(["']?)\s*(\d+(?:\.\d+)?%?)\s*\1(\s|$)/i,i=/(?:background|background-color|bgcolor)\s*[:=]\s*(["']?)\s*((rgb\s*\(\s*\d{1,3}%?,\s*\d{1,3}%?\s*,\s*\d{1,3}%?\s*\))|(#[0-9a-f]{3,6})|([a-z]{1,20}))\s*\1/i,a=String(f),k=[],m=0;a=a.replace(/\s*\r?\n\s*/g,"");a=a.replace(/<(script|style)(\s+[^>]*?)?>[\s\S]*?<\/\1>/ig,"");a=a.replace(/<!--[\s\S]*?--\>/ig,"");a=a.replace(/<br\s*?\/?>/ig,"\r\n");a=a.replace(/\[code\s*(=\s*([^\]]+?))?\]([\s\S]*?)\[\/code\]/ig,function(h){m++;k[m]=h;return"[\tubbcodeplace_"+
...
function(h,c,d,e){h=d.match(/(?:^|;)\s*font-family\s*:\s*([^;]+)/i);c=d.match(/(?:^|;)\s*font-size\s*:\s*([^;]+)/i);var j=d.match(/(?:^|;)\s*color\s*:\s*([^;]+)/i);d=d.match(/(?:^|;)\s*(?:background|background-color)\s*:\s*([^;]+)/i);e=e;if(h)e="[font="+h[1]+"]"+e+"[/font]";if(c)if(c=b[c[1].toLowerCase()])e="[size="+c+"]"+e+"[/size]";if(j)e="[color="+o(j[1])+"]"+e+"[/color]";if(d)e="[back="+o(d[1])+"]"+e+"[/back]";return e});for(f=0;f<3;f++)a=a.replace(/<(div|p)(?:\s+[^>]*?)?[\s"';]\s*(?:text-)?align\s*[=:]\s*(["']?)\s*(left|center|right)\s*\2[^>]*>(((?!<\1(\s+[^>]*?)?>)[\s\S])+?)<\/\1>/ig,
...
function(h,c,d,e){if(!(d&&e))return"";h="url";if(d.match(/^mailto:/i)){h="email";d=d.replace(/mailto:(.+?)/i,"$1")}c="["+h;if(d!=e)c+="="+d;return c+"]"+e+"[/"+h+"]"});a=a.replace(/<img(\s+[^>]*?)\/?>/ig,function(h,c){var d=c.match(/\s+emot\s*=\s*(["']?)\s*(.+?)\s*\1(\s|$)/i);if(d)return"[emot="+d[2]+"/]";d=c.match(p);var e=c.match(/\s+alt\s*=\s*(["']?)\s*(.*?)\s*\1(\s|$)/i),j=c.match(n),l=c.match(g),q=c.match(/\s+align\s*=\s*(["']?)\s*(\w+)\s*\1(\s|$)/i),r="[img",s="";if(!d)return"";s+=e[2];if(j||
l)s+=","+(j?j[2]:"")+","+(l?l[2]:"");if(q)s+=","+q[2];if(s)r+="="+s;r+="]"+d[2]+"[/img]";return r});a=a.replace(/<blockquote(?:\s+[^>]*?)?>([\s\S]+?)<\/blockquote>/ig,"[quote]$1[/quote]");a=a.replace(/<embed((?:\s+[^>]*?)?(?:\s+type\s*=\s*"\s*application\/x-shockwave-flash\s*"|\s+classid\s*=\s*"\s*clsid:d27cdb6e-ae6d-11cf-96b8-4445535400000\s*")[^>]*?)\/?>/ig,function(h,c){var d=c.match(p),e=c.match(n),j=c.match(g),l="[flash";if(!d)return"";if(e&&j)l+="="+e[2]+","+j[2];l+="]"+d[2];return l+"[/flash]"});
a=a.replace(/<embed((?:\s+[^>]*?)?(?:\s+type\s*=\s*"\s*application\/x-mplayer2\s*"|\s+classid\s*=\s*"\s*clsid:6bf52a52-394a-11d3-b153-00c04f79faa6\s*")[^>]*?)\/?>/ig,function(h,c){var d=c.match(p),e=c.match(n),j=c.match(g),l=c.match(/\s+autostart\s*=\s*(["']?)\s*(.+?)\s*\1(\s|$)/i),q="[media",r="0";if(!d)return"";if(l)if(l[2]=="true")r="1";if(e&&j)q+="="+e[2]+","+j[2]+","+r;q+="]"+d[2];return q+"[/media]"});a=a.replace(/<table(\s+[^>]*?)?>/ig,function(h,c){var d="[table";if(c){var e=c.match(n),j=
c.match(i);if(e){d+="="+e[2];if(j)d+=","+j[2]}}return d+"]"});a=a.replace(/<tr(\s+[^>]*?)?>/ig,function(h,c){var d="[tr";if(c){var e=c.match(i);if(e)d+="="+e[2]}return d+"]"});a=a.replace(/<(?:th|td)(\s+[^>]*?)?>/ig,function(h,c){var d="[td";if(c){var e=c.match(/\s+colspan\s*=\s*(["']?)\s*(\d+)\s*\1(\s|$)/i),j=c.match(/\s+rowspan\s*=\s*(["']?)\s*(\d+)\s*\1(\s|$)/i),l=c.match(n);e=e?e[2]:1;j=j?j[2]:1;if(e>1||j>1||l)d+="="+e+","+j;if(l)d+=","+l[2]}return d+"]"});a=a.replace(/<\/(table|tr)>/ig,"[/$1]");
a=a.replace(/<\/(th|td)>/ig,"[/td]");a=a.replace(/<ul(\s+[^>]*?)?>/ig,function(h,c){var d;if(c)d=c.match(/\s+type\s*=\s*(["']?)\s*(.+?)\s*\1(\s|$)/i);return"[list"+(d?"="+d[2]:"")+"]"});a=a.replace(/<ol(\s+[^>]*?)?>/ig,"[list=1]");a=a.replace(/<li(\s+[^>]*?)?>/ig,"[*]");a=a.replace(/<\/li>/ig,"");a=a.replace(/<\/(ul|ol)>/ig,"[/list]");a=a.replace(/<h([1-6])(\s+[^>]*?)?>/ig,function(h,c){return"\r\n\r\n[size="+(7-c)+"][b]"});a=a.replace(/<\/h[1-6]>/ig,"[/b][/size]\r\n\r\n");a=a.replace(/<address(\s+[^>]*?)?>/ig,
...
"\r\n\r\n");a=a.replace(/^((\s|&nbsp;)*\r?\n)+/g,"");a=a.replace(/((\s|&nbsp;)*\r?\n)+$/g,"");for(f=1;f<=m;f++)a=a.replace("[\tubbcodeplace_"+f+"\t]",k[f]);a=a.replace(/<[^<>]+?>/g,"");var t={lt:"<",gt:">",nbsp:" ",amp:"&",quot:'"'};return a=a.replace(/&(lt|gt|nbsp|amp|quot);/ig,function(h,c){return t[c]})};
